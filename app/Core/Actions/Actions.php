...
class Actions
{
    ...
    public function __construct(ActionsModel $model, Container $container)
    {
        ...
        while ((time() - $startTime) < $maxRunTime) {
            ...
            foreach ($actions as $key => $action) {
                ...
                try {
                    ...
                    if ($callback === ActionsClosure::class) {
                        if (!$payload instanceof ActionsClosure) {
                            throw new ActionsException('Invalid closure data.');
                        }
    ...
    protected function handleRecurring(array $action): void
    {
        ...
        if (is_numeric($recurringTime)) {
            ...
            if ($nextRun === false) {
                throw new ActionsException('Failed to resolve the recurring time: ' . $recurringTime);
            }
            ...
            if ($nextRun === false or $nextRun < $now) {
                throw new ActionsException('The recurring time is in the past: ' . $recurringTime);
            }
    ...
    protected function prepareCallbackData(callable|array|string $callback, array $args): array
    {
        if ($callback instanceof \Closure) {
            $wrapper = new ActionsClosure($callback, $args);
            $callbackKey = ActionsClosure::class;
        ...
        if (strlen($serialized) > 65000) {
            throw new ActionsException('Serialized closure is too long.');
        }
